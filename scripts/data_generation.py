...
parser.add_argument('-f', '--flow', default='amf', choices=['amf', 'convert', 'vivado', 'all'])
parser.add_argument('-l', '--log_dir')
parser.add_argument('-s', '--spread_iter', default="5", type=str)
parser.add_argument('--sampleid', default="1", type=str)
...
print('Augmented data would be generated at: ', bms)
...
def run_convert(bm_dir, bm_log_dir):
    print("convert pl to tcl", bm_log_dir)
    cmd = 'python macroplacement_bookshelf2vivado.py --bm_dir {0} --sol_dir {1} '.format(bm_dir, bm_log_dir)
    run(cmd)
...
for bm in bms:
    bm_log_dir = '{}/{}/AMF_solution/solution_{}'.format(log_dir, bm.full_name, args.sampleid)
    ...
    if args.flow == "all" or args.flow == "amf":
        # run amf
        # cmd = './Cumple {0}/mlcad.json | tee {0}/run.log'.format(bm_log_dir)
        cmd = './Cumple {0} {1} | tee {1}/run.log'.format(bm_path + bm.full_name, bm_log_dir)
        run(cmd)
        # run convert
        run_convert(conf['designPath'], bm_log_dir)

    if args.flow == "convert":
        run_convert(conf['designPath'], bm_log_dir)
